...
import data.utils
import data.service.rbac as context_data
...
allow {
	...
	country := context_data.users[input.user].location.country
	country == "US"
}
...
user_is_admin {
	...
	context_data.users[input.user].roles[i] == "admin"
}
...
user_is_granted[permission] {
	...
	role := context_data.users[input.user].roles[i]

	# `permission` assigned a single permission from the permissions list for 'role'...
	permission := context_data.role_permissions[role][j]
}